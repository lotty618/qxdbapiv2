...
public class FtpUtil {
    protected final static Logger log = LoggerFactory.getLogger(FtpUtil.class);
    ...
    public boolean downloadFile(String ftpPath, String fileName, String savePath) {
        ...
        if (ftpClient != null) {
            try {
                ...
                // 判断是否存在该目录
                if (!ftpClient.changeWorkingDirectory(new String(path.getBytes(),"ISO-8859-1"))) {
    ...
    public boolean downloadFiles(String ftpPath, String savePath) {
        ...
        if (ftpClient != null) {
            try {
                String path = changeEncoding(basePath + ftpPath);
                // 判断是否存在该目录
                if (!ftpClient.changeWorkingDirectory(new String(path.getBytes("GBK"),"ISO-8859-1"))) {
                    log.error(basePath + ftpPath + "该目录不存在");
                    return flag;
                }
                ftpClient.enterLocalPassiveMode();  // 设置被动模式，开通一个端口来传输数据
